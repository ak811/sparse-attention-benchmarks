...
@register_mask("fibottentioncrossdiag")
class FibottentionCrossDiagMask(AttentionMask):
    def __init__(self, add_class_token=True, modified=False, shuffled=True):
        self.add_class_token = add_class_token
        self.modified = modified
        self.shuffled = shuffled
        self._reported_epoch = None  # one-time-per-epoch sparsity print
    ...
    def __call__(self, attn, *, estep=None, N=None, num_heads=None, depth_id=None, device=None, **kwargs):
        ...
        if self._reported_epoch != epoch:
            ...
            print(
                f"[FibottentionCrossDiag][epoch={epoch}] modified={self.modified}, shuffled={self.shuffled}, "
...
def _wythoff(H, N, is_modified, is_shuffled, depth_id, add_class_token, device, dtype):
    """
    Build per-head masks with bands parallel to the SECONDARY (anti-) diagonal.
    """
    ...
    r = torch.arange(0, N, device=device)
    for h in range(H):
        for d in headindices[h]:
            c = (N - 1) - r + d
            keep = (c >= 0) & (c < N)
            if keep.any():
                mask[h, r[keep], c[keep].to(torch.long)] = 1

            c2 = (N - 1) - r - d
            keep2 = (c2 >= 0) & (c2 < N)
            if keep2.any():
                mask[h, r[keep2], c2[keep2].to(torch.long)] = 1
