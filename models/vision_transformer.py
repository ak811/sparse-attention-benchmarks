...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_norm=False, attn_drop=0., proj_drop=0., norm_layer=nn.LayerNorm, depth_id=0, attn_cfg=None):
        super().__init__()
        ...
        default_cfg={"backend":"vit","backend_kwargs":{},"mask":"sparse","mask_kwargs":{"pattern":"local","local_attn_ctx":64,"symmetric":True}}
        self.attn=build_attention_from_cfg(attn_cfg or default_cfg)
    ...
    def forward(self,x,estep):
        ...
        q=q*self.scale
        x_attn=self.attn(q,k,v,estep=estep,N=N,num_heads=self.num_heads,depth_id=self.depth_id,device=x.device,attn_drop=self.attn_drop,save_hook=save_intermediate_x)
        x_attn=x_attn.transpose(1,2).reshape(B,N,C)
        x_attn=self.proj(x_attn)
        return self.proj_drop(x_attn)
