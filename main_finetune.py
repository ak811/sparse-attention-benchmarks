...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # --- SAVE LAST AND BEST CHECKPOINT LOGIC ---
        if args.output_dir:
            # Always save the 'last' checkpoint for the current epoch
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, checkpoint_name="last"
            )
            
            # Check if this is the best accuracy so far, if so, save as 'best'
            if test_stats["acc1"] > max_accuracy:
                misc.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=epoch, checkpoint_name="best"
                )
        # -------------------------------------------
